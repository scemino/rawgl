...
void Mixer::stopMusic() { debug(DBG_SND, "Mixer::stopMusic()"); }

void Mixer::playAifcMusic(const char *path, uint32_t offset) {
  debug(DBG_SND, "Mixer::playAifcMusic(%s)", path);
}

void Mixer::stopAifcMusic() { debug(DBG_SND, "Mixer::stopAifcMusic()"); }

void Mixer::playSfxMusic(int num) {
  debug(DBG_SND, "Mixer::playSfxMusic(%d)", num);
}

void Mixer::stopSfxMusic() { debug(DBG_SND, "Mixer::stopSfxMusic()"); }

void Mixer::stopAll() { debug(DBG_SND, "Mixer::stopAll()"); }
